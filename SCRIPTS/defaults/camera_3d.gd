...
var debug_ray_immediate_mesh: ImmediateMesh
...
# NEW: Building placement mode tracking
var is_building_placement_active: bool = false
...
func _ready():
	...
	# Initialize and clamp zoom level correctly on startup
	...
	# Connect to BuildingPlacer if it exists
	_connect_to_building_placer()


func _connect_to_building_placer():
	"""Find and connect to BuildingPlacer to track placement mode"""
	# Try to find BuildingPlacer in parent or scene
	var parent = get_parent()
	while parent:
		if parent.has_node("BuildingPlacer"):
			var building_placer = parent.get_node("BuildingPlacer")
			if building_placer.has_signal("placement_mode_changed"):
				building_placer.placement_mode_changed.connect(_on_placement_mode_changed)
				if show_debug_print:
					ConsoleCapture.console_log("Camera: Connected to BuildingPlacer placement_mode_changed signal")
				return
		parent = parent.get_parent()
	
	# Alternative: search in scene root
	var root = get_tree().root
	for child in root.get_children():
		var building_placer = _find_building_placer_recursive(child)
		if building_placer:
			if building_placer.has_signal("placement_mode_changed"):
				building_placer.placement_mode_changed.connect(_on_placement_mode_changed)
				if show_debug_print:
					ConsoleCapture.console_log("Camera: Connected to BuildingPlacer via recursive search")
			return


func _find_building_placer_recursive(node: Node):
	"""Recursively search for BuildingPlacer node"""
	if node.get_script() and node.get_script().get_global_name() == "BuildingPlacer":
		return node
	
	for child in node.get_children():
		var result = _find_building_placer_recursive(child)
		if result:
			return result
...
func _on_placement_mode_changed(is_active: bool):
	"""Callback when building placement mode changes"""
	is_building_placement_active = is_active
	
	if show_debug_print:
		ConsoleCapture.console_log("Camera: Building placement mode %s - zoom %s" % ["ACTIVE" if is_active else "INACTIVE", "DISABLED" if is_active else "ENABLED"])
...
func _input(event):
	if event is InputEventMouseButton:
		...
		# ZOOM: Only allow zoom when NOT in building placement mode
		if not is_building_placement_active:
